...
import CampusOverviewTab from "../components/recruiter/CampusOverviewTab";
import DepartmentDeepDiveTab from "../components/recruiter/DepartmentDeepDiveTab";
import LanguageMatrixTab from "../components/recruiter/LanguageMatrixTab";
import ContestsEventsTab from "../components/recruiter/ContestsEventsTab";
import { 
  Building2, 
  ShieldCheck, 
  Search, 
...
export default function RecruiterConsole() {
  ...
  // Sub-Tab State: "overview" | "departments" | "languages" | "contests" | "talent"
  const [activeSubTab, setActiveSubTab] = useState<"overview" | "departments" | "languages" | "contests" | "talent">("overview");
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      console.warn("Failed loading recruiter analytics.");
    } finally {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.csv) {
          ...
          const link = document.createElement("a");
          link.href = url;
          link.download = `Lyndesk_${activeCompany.replace(/\s+/g, "_")}_Shortlist.csv`;
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        }
      ...
      console.warn("Export shortlist error:", err);
    }
  };

  // Filter computation for candidate roster
  const filteredCandidates = candidates.filter(c => {
    ...
    const matchesYear = yearFilter === "all" || c.academicYear.toLowerCase().includes(yearFilter.toLowerCase());
    const matchesSolved = minLcFilter === 0 || c.leetcodeSolved >= minLcFilter;
    const matchesVerified = !verifiedOnly || c.isVerified;

    return matchesSearch && matchesDept && matchesYear && matchesSolved && matchesVerified;
  });
    ...
    <div className="min-h-screen flex flex-col font-sans bg-bg-base text-txt-main">
      ...
      {!authorized ? (
        ...
        <main className="flex-1 flex items-center justify-center p-6 bg-bg-base">
          <div className="max-w-md w-full border border-border-main/60 bg-bg-surface p-8 rounded-sm shadow-xl flex flex-col gap-6">
            ...
            <div className="border-t border-border-main/40 pt-4 flex flex-col gap-2">
              <span className="text-[10px] font-mono text-txt-muted uppercase tracking-wider">Active Verified Access PINs:</span>
        ...
        <main className="flex-1 flex flex-col max-w-7xl w-full mx-auto p-4 sm:p-6 gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border-main/40 pb-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs text-txt-sub">
                Explore campus-wide engineering intelligence, department benchmarks, language matrices, and verified talent pools.
              </p>
            </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto border-b border-border-main/50 pb-2">
            <button
              onClick={() => setActiveSubTab("overview")}
              className={`px-3.5 py-2 rounded-sm text-xs font-mono uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer ${
                activeSubTab === "overview"
                  ? "bg-accent-main text-bg-base font-bold shadow-xs"
                  : "text-txt-sub hover:text-txt-main hover:bg-bg-surface"
              }`}
            >
              <Building2 size={14} /> Institutional Overview
            </button>
            ...
            >
              <Layers size={14} /> Department Deep-Dive
            </button>
            ...
            >
              <Code2 size={14} /> Language &amp; Stack Matrix
            </button>
            ...
            >
              <Trophy size={14} /> Contests &amp; Hackathons
            </button>
            ...
            >
              <Users size={14} /> Candidate Talent Roster
            </button>
          </div>
          ...
          {activeSubTab === "overview" && (
            <CampusOverviewTab 
              data={analyticsData?.overview || null} 
              institutionName={analyticsData?.institution?.name || "SRM Institute of Science and Technology"} 
              loading={loadingAnalytics} 
            />
          )}

          {/* ── SUB-TAB 2: DEPARTMENT DEEP-DIVE ── */}
          {activeSubTab === "departments" && (
            <DepartmentDeepDiveTab 
              departments={analyticsData?.departmentAnalytics || []} 
              campusAvgSolved={analyticsData?.overview?.avgSolved || 0} 
              loading={loadingAnalytics} 
            />
          )}

          {/* ── SUB-TAB 3: LANGUAGE & STACK MATRIX ── */}
          {activeSubTab === "languages" && (
            <LanguageMatrixTab 
              languages={analyticsData?.languageMatrix || []} 
              departments={analyticsData?.departmentAnalytics || []} 
              loading={loadingAnalytics} 
            />
          )}

          {/* ── SUB-TAB 4: CONTESTS & HACKATHONS ── */}
          {activeSubTab === "contests" && (
            <ContestsEventsTab 
              data={analyticsData?.contestAnalytics || null} 
              loading={loadingAnalytics} 
            />
          )}

          {/* ── SUB-TAB 5: CANDIDATE TALENT ROSTER ── */}
          {activeSubTab === "talent" && (
            <div className="flex flex-col gap-6">
              ...
              <div className="p-4 border border-border-main/60 bg-bg-surface rounded-sm flex flex-col md:flex-row gap-3 items-center justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
                  <select
                    ...
                    className="h-9 bg-bg-base border border-border-main px-3 text-xs text-txt-main focus:outline-none rounded-sm font-mono cursor-pointer"
                  >
                  ...
                  <label className="flex items-center gap-1.5 text-xs text-txt-sub font-mono cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-border-main text-accent-main focus:ring-0 cursor-pointer"
                    />
                    Verified Only
                  </label>
                </div>
              ...
              {/* Talent Matrix Table & Benchmark Card */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 border border-border-main/60 bg-bg-surface rounded-sm overflow-hidden flex flex-col">
                  <div className="p-3 border-b border-border-main/40 flex items-center justify-between font-mono text-[11px] text-txt-muted uppercase tracking-wider">
                    <span>Matching Anonymous Candidates ({filteredCandidates.length})</span>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="p-3 border border-border-main/50 bg-bg-base rounded-sm flex flex-col gap-1">
                          ...
                          <span className="text-[9px] text-emerald-400 font-mono">Top Cohort Solved</span>
                        </div>
                        ...
                        <div className="p-3 border border-border-main/50 bg-bg-base rounded-sm flex flex-col gap-1">
                          ...
                          <span className="text-[9px] text-txt-muted font-mono">Contest Specialist</span>
                        </div>
                        ...
                        <div className="p-3 border border-border-main/50 bg-bg-base rounded-sm flex flex-col gap-1">
                          ...
                          <span className="text-[9px] text-txt-muted font-mono">Division Rank</span>
                        </div>
                        ...
                        <div className="p-3 border border-border-main/50 bg-bg-base rounded-sm flex flex-col gap-1">
                          ...
                          <span className="text-[9px] text-txt-muted font-mono">Podiums</span>
                        </div>
                      </div>
                      ...
                      <div className="p-3 border border-border-main/40 bg-bg-base/50 rounded-sm text-[11px] text-txt-sub leading-relaxed font-light flex items-start gap-2">
                        ...
                        <span>
                          To request interview scheduling or candidate contact unlocking, reach out to the campus Placement cell referencing <strong>{selectedCandidate.candidateId}</strong>.
                        </span>
                      </div>
                    </div>
                  ) : (
